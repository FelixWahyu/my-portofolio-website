  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? "backdrop-blur-lg bg-gray-300/50 border-b border-white/10 shadow-lg" : "bg-transparent"}`}>
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
        <a href="#hero" className="text-2xl font-extrabold text-blue-500">
          Felix<span className="text-gray-800">WebDev</span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-8">
          {menuItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className={`relative text-gray-800 font-medium transition-colors
          ...
          <button onClick={toggleLanguage} className="px-4 py-1 rounded-full border border-blue-500/70 font-medium text-blue-400 hover:bg-blue-500 hover:text-white transition-all">
            {lang === "ID" ? "EN" : "ID"}
          </button>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-gray-300/50 backdrop-blur-xl px-6 py-4 space-y-4 border-t border-white/10">
          {menuItems.map((item) => (
            <a key={item.id} href={`#${item.id}`} onClick={() => setIsOpen(false)} className={`block font-medium text-lg transition-colors ${activeSection === item.id ? "text-blue-400" : "text-gray-800 hover:text-blue-400"}`}>
              {item.label}
            </a>
          ))}

          <button onClick={toggleLanguage} className="px-4 py-1 mt-3 rounded-full font-medium border border-blue-500/70 text-blue-400 hover:bg-blue-500 hover:text-white transition-all">
            {lang === "ID" ? "EN" : "ID"}
          </button>
        </div>
      )}
